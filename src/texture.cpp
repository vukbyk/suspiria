...
//    log_info("Loading texture: %s", "assets/defaultXY.png");

    std::string fileName="defaultXY.png";
    ...
    if (data == NULL)
;//        log_err("Unable to load texture: %s", "assets/defaultXY.png");
...
//{
////    textureTarget = aTextureTarget;
//    int width, height, nChannels;
////    log_info("Loading texture: %s", ("assets/"+fileName).c_str());
//    unsigned char *data = stbi_load(("assets/"+fileName).c_str(), &width, &height, &nChannels, 0);

//    if (data == NULL)
////    ;    log_err("Unable to load texture: %s", ("assets/"+fileName).c_str());
